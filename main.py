...
if __name__ == '__main__':
    if DATA_SOURCE == "MNIST":
        ...
        averaged_precision_recalls = [] # stored as a list of lists as [kappa, ave precision, ave recall]

        for kappa in KAPPAS:

            averaged_precision_recalls.append([kappa, 0, 0, 0, 0]) # kappa, ave precision, ave recall, ave baseline precision, ave baseline recall
            precisions = []
            recalls = []
            precision_baseline = []
            recall_baseline = []
            ...
            for seed in range(NUM_RUNS_TO_AVE):
                ...
                # X_skewed = X_skewed[:500]
                # Y_skewed = y_skewed[:500]
                # y_w_rel = y_w_rel[:500]
                ...
                num_total_points = len(data_stream.X)
                ...
                precisions.append(num_relevant_points_found / num_queries)  # Relevant points found / total query
                recalls.append(num_relevant_points_found / num_total_relevant_points)  # Relevant points found / total num rel points

                # Random Baseline
                # Note: see math explanation
                # Precision just equals ratio of num_relevant_pts to num_total_points
                precision_baseline.append(num_total_relevant_points / num_total_points)
                # Recall just equals query rate
                query_rate = num_queries/ num_total_points
                recall_baseline.append(query_rate)
            ...
            print(precisions, recalls, precision_baseline, recall_baseline)

            averaged_precision_recalls[-1][1] = sum(precisions) / len(precisions)
            averaged_precision_recalls[-1][2] = sum(recalls) / len(recalls)
            averaged_precision_recalls[-1][3] = sum(precision_baseline) / len(precision_baseline)
            averaged_precision_recalls[-1][4] = sum(recall_baseline) / len(recall_baseline)
            stats.store_ared_precision_recall(kappa, averaged_precision_recalls[-1][1], averaged_precision_recalls[-1][2], averaged_precision_recalls[-1][3], averaged_precision_recalls[-1][4])
